...
public class Serif_PlayStart : MonoBehaviour {

    Player_PlayStart player;
    ...
    {
        SetNextSentence();
        player = FindObjectOfType<Player_PlayStart>();
    }
    ...
    void Update()
    {
        ...
        if (IsDisplayComplete())
        {
            ...
            //最後の文章ではない & ボタンが押された
            if (currentSentenceNum < sentences.Length && Input.GetMouseButton(0))
            {
                SetNextSentence();
                player.MoveAnimation(a);
                one = true;
            }
        }
